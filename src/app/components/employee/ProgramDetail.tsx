...
const PROGRAM = {
  ...
  description: "Entrenamiento completo sobre seguridad contra incendios que cubre procedimientos de evacuación, uso de extintores y protocolos de respuesta a emergencias.",
  status: "en progreso",
  ...
  overallProgress: 65,
  content: [
    { id: 1, type: "simulation", name: "Evacuación de Incendios – Subterráneo", status: "aprobado", attempts: 2, maxAttempts: 3, bestScore: 88, locked: false },
    { id: 2, type: "simulation", name: "Respuesta a Derrame Químico", status: "en progreso", attempts: 1, maxAttempts: 3, bestScore: 62, locked: false },
    { id: 3, type: "exam", name: "Examen Teórico de Seguridad contra Incendios", status: "pendiente", attempts: 0, maxAttempts: 3, bestScore: null, locked: false },
    { id: 4, type: "simulation", name: "Evacuación de Incendios en Superficie", status: "bloqueado", attempts: 0, maxAttempts: 3, bestScore: null, locked: true },
    { id: 5, type: "exam", name: "Evaluación Final", status: "bloqueado", attempts: 0, maxAttempts: 2, bestScore: null, locked: true },
  ],
};
...
export function ProgramDetail() {
  ...
  return (
    <div>
      <Breadcrumb items={[
        { label: "Mi Entrenamiento", onClick: () => navigate("/employee") },
        { label: PROGRAM.name }
      ]} />
      ...
      <div className="grid grid-cols-3 gap-6 mb-6">
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <div className="flex items-center gap-4 text-xs" style={{ color: colors.textSecondary }}>
                ...
                <div>Coordinador: <span className="font-medium">{PROGRAM.coordinator}</span></div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1">
              <div className="flex justify-between text-xs mb-1">
                <span style={{ color: colors.textSecondary }}>Progreso General</span>
        ...
        <Card className="flex flex-col items-center justify-center">
          ...
          <p className="text-sm font-medium mt-3" style={{ color: colors.textPrimary }}>Finalización General</p>
          <p className="text-xs mt-1" style={{ color: colors.textSecondary }}>
            {PROGRAM.content.filter(c => c.status === "aprobado").length} de {PROGRAM.content.filter(c => !c.locked).length} elementos completados
          </p>
        </Card>
      </div>
      ...
      {/* Content List */}
      <Card className="mb-6">
        <h3 className="font-semibold mb-4" style={{ color: colors.textPrimary }}>Contenido del Entrenamiento</h3>
        <div className="space-y-3">
          {PROGRAM.content.map((item, i) => (
            <div
              key={item.id}
              className="flex items-center gap-4 p-4 rounded-xl border"
              style={{ borderColor: item.locked ? colors.border : item.status === "aprobado" ? `${colors.success}30` : colors.border, opacity: item.locked ? 0.6 : 1, backgroundColor: item.status === "aprobado" ? `${colors.success}05` : "white" }}
            >
              {/* Order number */}
              <div
                className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0"
                style={{ backgroundColor: item.status === "aprobado" ? colors.success : item.locked ? colors.border : `${colors.primary}15`, color: item.status === "aprobado" ? "white" : item.locked ? colors.textSecondary : colors.primary }}
              >
                {item.status === "aprobado" ? <CheckCircle size={14} /> : i + 1}
              </div>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs px-1.5 py-0.5 rounded capitalize" style={{ backgroundColor: colors.bg, color: colors.textSecondary }}>{item.type === "simulation" ? "simulación" : "examen"}</span>
                </div>
                <div className="flex items-center gap-3 mt-1 text-xs" style={{ color: colors.textSecondary }}>
                  {item.locked ? (
                    <span>Completa los elementos anteriores primero</span>
                  ) : (
                    <>
                      <span>{item.attempts}/{item.maxAttempts} intentos usados</span>
                      {item.bestScore !== null && (
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                ...
                ) : item.attempts >= item.maxAttempts && item.status !== "aprobado" ? (
                  <span className="text-xs px-3 py-1.5 rounded-lg" style={{ backgroundColor: `${colors.error}15`, color: colors.error }}>Sin intentos restantes</span>
                ) : item.type === "simulation" ? (
                  <button
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h3 className="font-semibold" style={{ color: allMandatoryApproved ? colors.success : colors.textPrimary }}>
              {allMandatoryApproved ? "¡Certificado Disponible!" : "Tu Certificado"}
            </h3>
