...
const EXAMS = [
  { id: 1, name: "Fire Safety Theory Exam", questions: 15, attempts: 3, usedIn: 2, status: "active" },
  { id: 2, name: "LOTO Knowledge Assessment", questions: 20, attempts: 2, usedIn: 1, status: "active" },
  { id: 3, name: "PPE Identification Test", questions: 10, attempts: 3, usedIn: 1, status: "active" },
  { id: 4, name: "Confined Space Pre-Entry Quiz", questions: 12, attempts: 2, usedIn: 0, status: "draft" },
];

const QUESTIONS = [
  { id: 1, text: "What is the first action to take when a fire alarm sounds underground?", category: "Fire Safety", options: 4, usedIn: 2 },
  { id: 2, text: "Which PPE is mandatory when entering a confined space?", category: "PPE", options: 4, usedIn: 1 },
  { id: 3, text: "How many steps are in the standard LOTO procedure?", category: "Energy", options: 4, usedIn: 1 },
  { id: 4, text: "What does IDLH stand for in confined space regulations?", category: "Confined Spaces", options: 4, usedIn: 1 },
  { id: 5, text: "When should PPE be inspected before use?", category: "PPE", options: 4, usedIn: 2 },
];
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="absolute right-0 top-0 bottom-0 w-[520px] bg-white shadow-2xl flex flex-col">
        <div className="flex items-center justify-between px-6 py-5 border-b" style={{ borderColor: colors.border }}>
          <h3 className="font-semibold" style={{ color: colors.textPrimary }}>New Question</h3>
          <button onClick={onClose}><X size={18} /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium mb-1" style={{ color: colors.textPrimary }}>Question Text *</label>
          ...
          <div>
            ...
            <input value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))}
              placeholder="e.g. Fire Safety, PPE, Energy"
              className="w-full px-3 py-2.5 rounded-lg border text-sm outline-none" style={{ borderColor: colors.border }} />
          </div>

          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-medium" style={{ color: colors.textPrimary }}>Answer Options *</label>
            ...
            <p className="text-xs mt-2" style={{ color: colors.textSecondary }}>Select the radio button next to the correct answer</p>
          </div>
        </div>
        <div className="px-6 py-4 border-t flex justify-end gap-3" style={{ borderColor: colors.border }}>
          ...
          <PrimaryBtn onClick={onClose}>Save Question</PrimaryBtn>
        </div>
      </div>
    </div>
  );
...
export function ExamManagement() {
  ...
  return (
    <div>
      <PageHeader
        title="Theoretical Exams"
        actions={
          activeTab === "exams"
            ? <PrimaryBtn><Plus size={16} /> New Exam</PrimaryBtn>
            : <PrimaryBtn onClick={() => setShowQuestionDrawer(true)}><Plus size={16} /> New Question</PrimaryBtn>
        }
      />

      <div className="flex gap-1 mb-6 p-1 bg-white rounded-xl border w-fit" style={{ borderColor: colors.border }}>
        {[{ key: "exams", label: "Exams" }, { key: "questions", label: "Question Bank" }].map(tab => (
          <button
            key={tab.key}
      ...
      {activeTab === "exams" ? (
        <div className="bg-white rounded-xl border overflow-hidden shadow-sm" style={{ borderColor: colors.border }}>
          <table className="w-full text-sm">
            <thead style={{ backgroundColor: colors.bg, borderBottom: `1px solid ${colors.border}` }}>
              <tr>
                {["Exam Name", "Questions", "Max Attempts", "Used In", "Status", "Actions"].map(h => (
            ...
            <tbody>
              {EXAMS.map((e, i) => (
                <tr key={e.id} style={{ borderBottom: i < EXAMS.length - 1 ? `1px solid ${colors.border}` : "none" }}
                  ...
                  <td className="px-5 py-3.5" style={{ color: colors.textSecondary }}>{e.usedIn} program{e.usedIn !== 1 ? "s" : ""}</td>
        ...
        <div className="bg-white rounded-xl border overflow-hidden shadow-sm" style={{ borderColor: colors.border }}>
          <table className="w-full text-sm">
            <thead style={{ backgroundColor: colors.bg, borderBottom: `1px solid ${colors.border}` }}>
              <tr>
                {["Question", "Category", "Options", "Used In", "Actions"].map(h => (
                  <th key={h} className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: colors.textSecondary }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {QUESTIONS.length === 0 ? (
                <tr><td colSpan={5}><EmptyState title="No questions yet" subtitle="Create your first question using the button above" action={<PrimaryBtn onClick={() => setShowQuestionDrawer(true)}><Plus size={16} /> New Question</PrimaryBtn>} /></td></tr>
              ) : QUESTIONS.map((q, i) => (
                <tr key={q.id} style={{ borderBottom: i < QUESTIONS.length - 1 ? `1px solid ${colors.border}` : "none" }}
                  ...
                  <td className="px-5 py-3.5" style={{ color: colors.textSecondary }}>{q.usedIn} exam{q.usedIn !== 1 ? "s" : ""}</td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2">
                      <button className="p-1.5 rounded-lg hover:bg-gray-100"><Pencil size={15} style={{ color: colors.textSecondary }} /></button>
