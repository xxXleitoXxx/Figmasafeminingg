...
import { PageHeader, PrimaryBtn, OutlinedBtn, InputField, SectionLabel, Card, Breadcrumb, Toast, Avatar, colors } from "../shared";
...
export function CompanyCreate() {
  ...
  const [admins, setAdmins] = useState<typeof AVAILABLE_ADMINS>([]);
  ...
  const validate = () => {
    ...
    } else if (form.cuit === "30-71234567-8") { // Mock check for CA4
      newErrors.cuit = "El CUIT ya existe en otra Empresa";
    }

    if (!form.email) {
      newErrors.email = "El Correo Electrónico de Contacto es obligatorio";
  ...
  return (
    <div>
      ...
      <PageHeader
        title="Nueva Empresa"
        subtitle="Registrar una nueva empresa en la plataforma"
        actions={
          <>
            <OutlinedBtn onClick={() => navigate("/admin/companies")}>Cancelar</OutlinedBtn>
            <PrimaryBtn onClick={handleSave}>Crear Empresa</PrimaryBtn>
          </>
        }
      />
      ...
      <div className="max-w-3xl space-y-6">
        <Card>
          ...
          <div className="grid grid-cols-2 gap-5 mt-4">
            ...
            />
            <InputField 
              label="Trade Name (Nombre Comercial)" 
